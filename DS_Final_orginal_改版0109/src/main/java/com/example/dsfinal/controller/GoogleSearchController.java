...
public class GoogleSearchController {
    ...
    public String search(@RequestParam("query") String query, Model model) {
        try {
            ...
            //HashMap<String, String> searchResults = (HashMap<String, String>) results.get("searchResults");
            //List<String> relatedKeywords = (List<String>) results.get("relatedKeywords");
            //List<WebNode> sortedResults = queryService.sortResults(searchResults);
            //model.addAttribute("results", sortedResults);  // 添加查詢結果到模型中
            //model.addAttribute("relatedKeywords", relatedKeywords); // 添加相關關鍵字到模型中
